...
@click.option('--outdir', default="/tmp/vcf_snps_utils_output", help='set the output directory, default is "/tmp/vcf_snps_utils_output/"')
@pass_context
def cli(ctx, outdir, vcffiles, csvfile):
    ...
    for vcffile in vcffiles:
        ...
        output_txt_files = create_output_files(results, outdir, ref_samples)
    ...
    for ref_sample in ref_samples:
        ...
        call('intervene upset -i {0}/{1}.txt {0}/vcf/*.txt --type list --output {2}'.format(outdir, ref_sample, sample_output_dir), shell=True, stdout=PIPE)
        ctx.log("## End")
